...
export default function CallLogDetailsPage() {
  ...
  const fetchVisitorLog = async () => {
    try {
      const response = await visitorLogService.getById(id);
      if (response?.data) {
  ...
  return (
    <div className="">
      ...
      <div className="flex items-center gap-3 mb-6">
        ...
        <h1 className="text-3xl font-bold">Visitor Log Details</h1>
      </div>
          ...
          <Tab.Panels className="p-6">
            ...
            <Tab.Panel>
              <div className="grid grid-cols-1 md:grid-cols-3 gap-6">
                <DetailRow label="Visitor Name" value={callLog.visitor_name} />
                <DetailRow
                  label="Contact Number"
                  value={callLog.visitor_contact_number}
                />
                <DetailRow
            ...
            <Tab.Panel>
              <div className="grid grid-cols-1 md:grid-cols-3 gap-6">
                ...
                />
                <DetailRow label="Date" value={formatDate(callLog.date)} />
              </div>
            </Tab.Panel>
...
const formatDate = (dateString: string) => {
  return dateString ? dayjs(dateString).format("DD-MMM-YYYY HH:mm") : "N/A";
};
