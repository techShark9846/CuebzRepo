...
//   const mainRows = table.getCoreRowModel().rows;
...
function TableHeadBasic<TData extends Record<string, any>>({
  headerGroup,
  isLeftScrollable,
  isRightScrollable,
  className,
          ...
          >
            <Box className="flex items-center">
              ...
              {header.column.getCanSort() ? (
                ...
                >
                  {{
                    asc: <PiCaretUpFill size={12} />,
                    desc: <PiCaretDownFill size={12} />,
...
function TableCellBasic<TData extends Record<string, any>>({
  cell,
  isLeftScrollable,
  isRightScrollable,
  className,
  ...
  return (
    <Table.Cell
      ...
      className={cn(
        "text-xs text-gray-800 px-4 py-2",
        isPinned && canPin && "sticky z-10",
        !isPinned && canResize && "relative",
        isPinned && isLeftScrollable && "sticky-right",
        isPinned && isRightScrollable && "sticky-left",
...
function PinnedRow<TData extends Record<string, any>>({
  row,
  table,
  isLeftScrollable,
  isRightScrollable,
        ...
        >
          <Fragment>
            <Table.Header
              className={cn(
                stickyHeader &&
                  "sticky top-0 z-10 bg-[#F4F4F5] text-sm font-semibold text-gray-800",
                classNames?.headerClassName
              )}
            >
              {table.getHeaderGroups().map((headerGroup) => (
          ...
          <Table.Body className={classNames?.bodyClassName}>
            ...
            {mainRows.map((row) => (
              <Fragment key={row.id}>
                <Table.Row className="hover:bg-gray-50 border-b border-gray-200">
                  {row.getVisibleCells().map((cell) => (
                    <TableCellBasic
                      key={cell.id}
                      cell={cell}
                      isLeftScrollable={isLeftScrollable}
                      isRightScrollable={isRightScrollable}
                      className={"px-4 py-2 " + classNames?.cellClassName}
                    />
                  ))}
                </Table.Row>
              </Fragment>
